...
class BBcSignature:
    ...
    def verify(self, digest):
        ...
        if self.keypair is None:
            _set_error(code=bbc_error.EBADKEYPAIR, txt="Bad private_key/public_key")
            return False
        try:
    ...
    def sign(self, key_type=DEFAULT_CURVETYPE, private_key=None, public_key=None, keypair=None):
        ...
        if keypair is None:
            if len(private_key) != 32 or len(public_key) <= 32:
                _set_error(code=bbc_error.EBADKEYPAIR, txt="Bad private_key/public_key (must be in bytes format)")
                return None
        ...
        if s is None:
            _set_error(code=bbc_error.EOTHER, txt="sig_type %d is not supported" % keypair.curvetype)
            return None
...
class BBcEvent:
    ...
    def __str__(self):
        ret =  "  asset_group_id: %s\n" % _str_binary(self.asset_group_id)
...
class BBcAsset:
    ...
    def __str__(self):
        ret =  "  Asset:\n"
        ret += "     asset_id: %s\n" % _str_binary(self.asset_id)
        ret += "     user_id: %s\n" % _str_binary(self.user_id)
        ret += "     nonce: %s\n" % _str_binary(self.nonce)
        ret += "     file_size: %d\n" % self.asset_file_size
        ret += "     file_digest: %s\n" % _str_binary(self.asset_file_digest)
