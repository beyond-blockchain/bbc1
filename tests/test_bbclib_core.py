...
import sys
sys.path.extend(["../"])
from bbc1.core.bbclib_core import BBcTransaction, BBcEvent, BBcReference, BBcWitness, BBcRelation, BBcAsset, BBcCrossRef, KeyPair
from bbc1.core import bbclib, bbclib_core

user_id = bbclib_core.get_new_id("user_id_test1")
user_id2 = bbclib_core.get_new_id("user_id_test2")
domain_id = bbclib_core.get_new_id("testdomain")
asset_group_id = bbclib_core.get_new_id("asset_group_1")
transaction1_id = bbclib_core.get_new_id("transaction_1")
transaction2_id = bbclib_core.get_new_id("transaction_2")
keypair1 = KeyPair()
keypair1.generate()
keypair2 = KeyPair()
keypair2.generate()
...
class TestBBcLib(object):
    ...
    def test_03_transaction_1(self):
        ...
        if sig is None:
            print(bbclib_core.error_text)
            assert sig
        transaction1.add_signature(signature=sig)

        # --- for checking serialization function ---
        digest = transaction1.digest()
        dat = bbclib.serialize(transaction1)
        print("Digest:", binascii.b2a_hex(digest))
        print("Serialized data:", binascii.b2a_hex(dat))

        transaction_tmp, fmt_type = bbclib.deserialize(dat)
    ...
    def test_06_transaction_with_relation_and_witness(self):
        print("\n-----", sys._getframe().f_code.co_name, "-----")
        transaction1 = bbclib_core.make_transaction(relation_num=1, witness=True)
        bbclib_core.add_relation_asset(transaction1, relation_idx=0, asset_group_id=asset_group_id,
                                       user_id=user_id, asset_body=b'ccccc')
        bbclib_core.add_relation_pointer(transaction1, 0, ref_transaction_id=transaction2.digest())
