...
client_num = 10
cores = None
client_socks = [None for i in range(core_nodes)]
...
class TestBBcNetworkWithCore(object):
    ...
    def test_01_start(self):
        ...
        for i in range(core_nodes):
            client_socks[i] = socket.create_connection(("127.0.0.1", 9000 + i))
            cores[i].networking.create_domain(domain_id=domain_id)
            nodes[i] = cores[i].networking.domains[domain_id]["node_id"]
            cores[i].networking.domains[domain_id][InfraMessageCategory.CATEGORY_USER].register_user(users[i], client_socks[i])
