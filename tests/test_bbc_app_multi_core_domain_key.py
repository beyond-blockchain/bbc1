...
class TestBBcAppClient(object):
    ...
    def test_00_setup(self):
        ...
        global domain_keyname
        domain_keypair = bbclib.KeyPair()
        domain_keypair.generate()
        domain_keyname = os.path.join(".bbc1-domainkeys", domain_id.hex() + ".pem")
        try:
            os.mkdir(".bbc1-domainkeys")
        except:
            pass
        with open(domain_keyname, "wb") as f:
            f.write(domain_keypair.get_private_key_in_pem())

        with open("conf_for_domainkey_test.json", "wb") as f:
            f.write(json.dumps(config_file_content).encode())

        global msg_processor
        prepare(core_num=core_num, client_num=client_num,
                conf_file="conf_for_domainkey_test.json", loglevel=LOGLEVEL)
        ...
        time.sleep(1)
        os.remove("conf_for_domainkey_test.json")
    ...
    def test_01_set_domain_key(self):
        print("\n-----", sys._getframe().f_code.co_name, "-----")
        # clients[0]['app'].set_node_key(".bbc1-9000/node_key.pem")  # node_key.pem is created by bbc_core

        for i in range(1, client_num):
            clients[i]['app'].set_node_key(".bbc1-900%d/node_key.pem" % i)  # node_key.pem is created by bbc_core
